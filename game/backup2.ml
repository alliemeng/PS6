...
let pick_cheap_mon (g:game) (mon_cost: int) : steammon = 
  ...
  else (cost,acc)) g.mon_table (mon_cost,[]) in 
  List.hd cheap_mons
...
let handle_step (g:game) (ra:command) (ba:command) : game_output =
  match ra, ba with
    ...
    | DoNothing, Action (SendTeamName blue_name) -> 
      ...
      send_update (InitGraphics ("Red",blue_name));    
      handle_teamname g   

    | Action(SendTeamName red_name), DoNothing -> 
      
      send_update (InitGraphics (red_name,"Blue"));    
      handle_teamname g   
 ...
 *)    
    ...
    | DoNothing, DoNothing -> 
      ...
      | PickRequest c -> handle_draft_action g c ""
      | PickInventoryRequest -> 
        begin
          set_inventory g Blue [];
          set_inventory g Blue [];
          last_request_sent := StarterRequest;  
          let game_data = game_datafication g in 
          (None, game_data, Some(Request(StarterRequest(game_data))),
      ...
      | StarterRequest -> 
        begin
          send_update (SetChosenSteammon (List.hd g.red.mon_list).species);
          send_update (SetChosenSteammon (List.hd g.blue.mon_list).species);
