...
let handle_step (g:game) (ra:command) (ba:command) : game_output =
  ...
  (* match ra, ba with
    ...
    | DoNothing, DoNothing -> 
      (match !last_request_sent with 
      | TeamNameRequest -> handle_SendTeamName g "Red" "Blue" 
      | PickRequest c -> handle_PickSteammon g c ""
      | PickInventoryRequest -> handle_PickInventory g [] []
      | StarterRequest -> handle_SelectStarter g "" "" 
      | ActionRequest -> 
        let game_data = game_datafication g in 
        (None, game_data, Some(Request(ActionRequest game_data)),
          Some(Request(ActionRequest game_data))))
