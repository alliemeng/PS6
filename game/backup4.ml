...
(* Sends a StarterRequest output for [c] if [c] player's active 
 * steammon has fainted. Does not change last_request_sent. *)
let switch_fainted_helper (g:game) (c:color) :game_output = 
  let game_data = game_datafication g in 
  match c with
  | Red -> (None, game_data, Some(Request(StarterRequest(game_data))),
    None)
  | Blue -> (None, game_data, None,
    Some(Request(StarterRequest(game_data))))
 ...
 *               [c] player's starter *)
let handle_fainted (g:game) (c:color) (starter:string) : game_output = 
  set_starter g c starter;

  let game_data = game_datafication g in 
  (None, game_data, Some(Request(ActionRequest game_data)),
    Some(Request(ActionRequest game_data)))

module BattleFn = functor (G:STATE) -> struct
  type t = {mutable r_mon: steammon; mutable b_mon: steammon}

  let create () = 
    {r_mon = List.hd G.red.mon_list; b_mon = List.hd G.blue.mon_list}
  
  let move (m:move) (c:color) = 
    match c with
    | Red -> 
    | Blue -> 
...
let handle_step (g:game) (ra:command) (ba:command) : game_output =
  match !last_request_sent with 
    ...
    | ActionRequest -> (match ra, ba with
      (* Occurs when steammon faints*)
      | Action(SelectStarter starter), DoNothing -> handle_fainted g Red starter
      | DoNothing, Action(SelectStarter starter) -> handle_fainted g Blue starter

      | Action(r_action), Action(b_action) ->  
