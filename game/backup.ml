 ...
 *)        
    ...
    | DoNothing, Action(PickSteammon steammon) ->
      ...
      (* | Action(UseMove red_move), Action (UseMove blue_move) -> 
       ...
       ( let red_active = List.hd g.red.mon_list in
        let blue_active = List.hd g.blue.mon_list in 
        (*Determine who goes first*)
        if red_active.speed > blue_active.speed then 
          (*resolve red's action first*)
          send_update (SetFirstAttacker Red) 

        else if red_active.speed < blue_active.speed then
         (*resolve blue's action first*)
          send_update (SetFirstAttacker Blue) 

        else (*choose order of resolution randomly*)
